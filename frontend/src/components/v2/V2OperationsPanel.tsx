...
import type { V2ActivityEvent, V2AgentMessage, V2ArtifactKind, V2MechanismState, V2ProviderId, V2ProviderStatus } from "@/components/v2/types";
import { V2GuidedMissionChecklist } from "@/components/v2/V2GuidedMissionChecklist";

export function V2OperationsPanel({ state, activeTask, onOpenReports, activeArtifact, onOpenArtifact, onCloseArtifact, onCommand, activeProvider, providers, activityLog, latestError, messages }: { state: V2MechanismState; activeTask: string; onOpenReports: () => void; activeArtifact: V2ArtifactKind | null; onOpenArtifact: (kind: V2ArtifactKind) => void; onCloseArtifact: () => void; onCommand: (command: string) => void; activeProvider: V2ProviderId; providers: V2ProviderStatus[]; activityLog: V2ActivityEvent[]; latestError: string | null; messages: V2AgentMessage[] }) {
  ...
  return <aside className="v2-scrollbar mt-3 h-auto overflow-y-auto rounded-[1.4rem] border border-v2-border bg-[#080807] p-3 xl:sticky xl:top-2 xl:mt-0 xl:block xl:h-[calc(100vh-1rem)]">
    ...
    <Panel title="Compact Inspector"><V2CompactInspector state={state} onOpenArtifact={onOpenArtifact} onCommand={onCommand} /><p className="mt-2 text-[10px] uppercase tracking-[0.18em] text-v2-muted">Drawer: {activeArtifact ?? "closed"}</p>{activeArtifact ? <button onClick={onCloseArtifact} className="mt-2 rounded-xl border border-v2-border bg-[#191715] px-3 py-2 text-xs font-semibold text-v2-muted hover:text-amber-500">Close drawer</button> : null}</Panel>
    <Panel title="Mission Checklist"><V2GuidedMissionChecklist state={state} messages={messages} /></Panel>
