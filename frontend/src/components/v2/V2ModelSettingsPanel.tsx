...
export function V2ModelSettingsPanel() {
  ...
  useEffect(() => {
    fetch("/api/v2/models/connect")
      ...
      .then((json: { providers?: ProviderInfo[] }) => { if (json.providers?.length) setProviders(json.providers); })
      .catch(() => undefined);
  }, []);
...
function StatusDot({ status }: { status: ProviderStatus }) {
  const color = status === "connected" ? "bg-amber-500" : status === "checking" ? "bg-blue-400" : status === "error" ? "bg-red-500" : "bg-stone-700";
