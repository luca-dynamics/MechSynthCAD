...
export function V2CommandDock({ onSubmit }: { onSubmit: (command: string, modelProvider?: string) => void }) {
  const [value, setValue] = useState("");
  const [modelProvider, setModelProvider] = useState("local");
  function submit(event: FormEvent) { event.preventDefault(); const command = value.trim(); if (!command) return; onSubmit(command, modelProvider); setValue(""); }
  return <form onSubmit={submit} className="rounded-[1.65rem] border border-v2-border bg-[#050505] p-3 shadow-[0_24px_80px_rgba(0,0,0,0.45)]">
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2">{chips.map((chip) => <button type="button" key={chip} className="rounded-full border border-v2-border bg-[#121110] px-3 py-1.5 text-[11px] font-semibold text-v2-muted hover:border-amber-600/40 hover:text-amber-500">{chip}</button>)}<select value={modelProvider} onChange={(event) => setModelProvider(event.target.value)} className="ml-auto rounded-full border border-v2-border bg-[#121110] px-3 py-1.5 text-[11px] text-v2-muted"><option value="local">Local agent</option><option value="openai">OpenAI · Dev/BYOK</option><option value="anthropic">Anthropic · Dev/BYOK</option><option value="gemini">Gemini · Dev/BYOK</option></select><button className="rounded-full bg-amber-600 px-5 py-2 text-sm font-bold text-black hover:bg-amber-500">Send</button></div>
  </form>;
}
