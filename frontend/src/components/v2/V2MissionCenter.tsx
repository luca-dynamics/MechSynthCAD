...
export function V2MissionCenter({ active, state, messages, activeTask, onCommand, onNavigate, theme, onThemeChange, activeProvider, providers, onProviderSelect, onMechanismSelect }: { active: V2NavItem; state: V2MechanismState; messages: V2AgentMessage[]; activeTask: string; onCommand: (command: string, modelProvider?: V2ProviderId) => void; onNavigate: (item: V2NavItem) => void; theme: V2Theme; onThemeChange: (theme: V2Theme) => void; activeProvider: V2ProviderId; providers: V2ProviderStatus[]; onProviderSelect: (provider: V2ProviderId) => void; onMechanismSelect: (mechanism: import("@/types").MechanismType) => void }) {
  ...
  return <section className="v2-scrollbar h-[calc(100vh-1rem)] overflow-y-auto rounded-[1.4rem] border border-v2-border bg-[#0d0c0b] p-4 pb-36">
    ...
    <div className="mt-4"><V2CanvasArtifact state={state} onNavigate={onNavigate} /></div>
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-3"><V2ArtifactCard title="Deterministic Result" status={state.solverResult ? "ready" : "waiting"} summary={state.solverResult ? "Backend solver output is available for interpretation." : "Run analysis to create a result artifact."} action={state.solverResult ? "Open results" : "Run analysis"} onAction={() => state.solverResult ? onNavigate("Results") : onCommand("Run analysis")} /><V2ArtifactCard title="Sweep Simulation" status={sampleCount ? "ready" : "idle"} summary={sampleCount ? `${sampleCount} deterministic samples stored.` : "Run a sweep to populate motion samples."} action="Run simulation" onAction={() => onCommand("Run simulation")} /><V2ArtifactCard title="Report Draft" status="available" summary="Build Markdown, print, or save PDF from current mission evidence." action="Open reports" onAction={() => onNavigate("Reports")} /><V2ArtifactCard title="Validation Matrix" status="available" summary="Review source-of-truth rules, solver boundaries, and checks." action="Open validation" onAction={() => onNavigate("Validation")} /><V2ArtifactCard title="Synthesis Recommendation" status={state.latestSynthesisRecommendations ? "ready" : "idle"} summary="Open synthesis recommendations from deterministic result context." action="Open synthesis" onAction={() => onNavigate("Results")} /></div>
