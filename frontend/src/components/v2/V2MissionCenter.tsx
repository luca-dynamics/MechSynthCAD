...
import type { V2AgentMessage, V2MechanismState, V2NavItem, V2ProviderId, V2ProviderStatus } from "@/components/v2/types";
...
const drawerTitles: Record<ArtifactKind, string> = { canvas: "CAD Preview", parameters: "Parameter Intake", result: "Deterministic Result", simulation: "Sweep Simulation", report: "Report Draft", validation: "Validation Matrix", synthesis: "Synthesis Recommendation" };

export function V2MissionCenter({ active, state, messages, activeTask, onCommand, onNavigate, theme, onThemeChange, activeProvider, providers, onProviderSelect, onMechanismSelect }: { active: V2NavItem; state: V2MechanismState; messages: V2AgentMessage[]; activeTask: string; onCommand: (command: string, modelProvider?: V2ProviderId) => void; onNavigate: (item: V2NavItem) => void; theme: V2Theme; onThemeChange: (theme: V2Theme) => void; activeProvider: V2ProviderId; providers: V2ProviderStatus[]; onProviderSelect: (provider: V2ProviderId) => void; onMechanismSelect: (mechanism: import("@/types").MechanismType) => void }) {
  const [artifact, setArtifact] = useState<ArtifactKind | null>(null);
  useEffect(() => {
    const openArtifact = (event: Event) => setArtifact((event as CustomEvent<ArtifactKind>).detail);
    window.addEventListener("v2-open-artifact", openArtifact);
    return () => window.removeEventListener("v2-open-artifact", openArtifact);
  }, []);
  ...
  return <section className="v2-scrollbar h-[calc(100vh-1rem)] overflow-y-auto rounded-[1.4rem] border border-v2-border bg-[#0d0c0b] p-4 pb-36">
    ...
    <div className="mt-4"><V2CanvasArtifact state={state} onOpenArtifact={() => setArtifact("canvas")} /></div>
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-3">
      <V2ArtifactCard title="CAD Preview" status="available" summary="Compact engineering canvas with a focused artifact drawer." action="Open canvas" onAction={() => setArtifact("canvas")} />
      <V2ArtifactCard title="Deterministic Result" status={state.solverResult ? "ready" : "waiting"} summary={state.solverResult ? "Backend solver output is available for interpretation." : "Run analysis to create a result artifact."} action={state.solverResult ? "Open result" : "Run analysis"} onAction={() => state.solverResult ? setArtifact("result") : onCommand("Run analysis")} />
      <V2ArtifactCard title="Sweep Simulation" status={sampleCount ? "ready" : "idle"} summary={sampleCount ? `${sampleCount} deterministic samples stored.` : "Run a sweep to populate motion samples."} action={sampleCount ? "Open simulation" : "Run simulation"} onAction={() => sampleCount ? setArtifact("simulation") : onCommand("Run simulation")} />
      <V2ArtifactCard title="Report Draft" status="available" summary="Build Markdown, print, or save PDF from current mission evidence." action="Open report" onAction={() => setArtifact("report")} />
      <V2ArtifactCard title="Validation Matrix" status="available" summary="Review source-of-truth rules, solver boundaries, and checks." action="Open validation" onAction={() => setArtifact("validation")} />
      <V2ArtifactCard title="Synthesis Recommendation" status={state.latestSynthesisRecommendations ? "ready" : "idle"} summary={state.solverResult ? "Open synthesis recommendations from deterministic result context." : "Run analysis before synthesis recommendations."} action={state.solverResult ? "Open synthesis" : "Run analysis"} onAction={() => state.solverResult ? setArtifact("synthesis") : onCommand("Run analysis")} />
    </div>
    ...
    <V2ArtifactDrawer open={Boolean(artifact)} title={artifact ? drawerTitles[artifact] : "Artifact"} subtitle="Focused V2 detail view" onClose={() => setArtifact(null)}>{artifact ? <V2ArtifactDetail kind={artifact} state={state} messages={messages} onCommand={onCommand} /> : null}</V2ArtifactDrawer>
  </section>;
}
