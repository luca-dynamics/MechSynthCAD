...
import { fourBarDemoParameters, fourBarDemoSweep, getV2MissionTemplate, sliderCrankDemoParameters, sliderCrankDemoSweep, type V2MissionTemplateId } from "@/components/v2/missionTemplates";
...
export function V2AppShell() {
  ...
  useEffect(() => { if (window.localStorage.getItem(firstRunStorageKey) !== "true") setGuideOpen(true); }, []);
  ...
  const resetMissionState = useCallback(() => {
    setSelectedMechanism("four_bar"); setForm(initialForm); setSweepForm(initialSweep); setResult(null); setSweepResult(null); setSelectedSampleIndex(0); setIsPlaying(false); setError(null);
    setSliderCrankForm(initialSliderCrankForm); setSliderCrankSweepForm(initialSliderCrankSweep); setSliderCrankResult(null); setSliderCrankSweepResult(null); setSelectedSliderCrankSampleIndex(0); setIsSliderCrankPlaying(false); setSliderCrankError(null);
    setLatestSynthesisRecommendations(null); setActiveArtifact(null);
    setMessages([buildAgentMessage("V2 mission state reset. Load a demo or choose a mechanism to continue.", "help", [completeStep("Mission reset", "V2 workspace state cleared; classic workspace is unaffected.")], defaultActions, "local", "completed", "four_bar")]);
    setActivityLog([buildActivity("system", "V2 mission state reset.", "success")]);
  }, []);

  const closeGuide = useCallback((_neverShowAgain?: boolean) => { window.localStorage.setItem(firstRunStorageKey, "true"); setGuideOpen(false); }, []);

  const startMissionTemplate = useCallback((templateId: V2MissionTemplateId) => {
    const template = getV2MissionTemplate(templateId); if (!template) return;
    if (template.mechanism === "four_bar") { setSelectedMechanism("four_bar"); setForm(fourBarDemoParameters); setSweepForm(fourBarDemoSweep); setResult(null); setSweepResult(null); setSelectedSampleIndex(0); }
    if (template.mechanism === "slider_crank") { setSelectedMechanism("slider_crank"); setSliderCrankForm(sliderCrankDemoParameters); setSliderCrankSweepForm(sliderCrankDemoSweep); setSliderCrankResult(null); setSliderCrankSweepResult(null); setSelectedSliderCrankSampleIndex(0); }
    setLatestSynthesisRecommendations(null); setActive("Workspace"); if (template.targetArtifact) setActiveArtifact(template.targetArtifact);
    const mechanism = template.mechanism ?? selectedMechanism;
    setMessages((thread) => [...thread, buildAgentMessage(`${template.shortTitle} loaded. Review parameters, then run analysis.`, "help", template.expectedSteps.map((step, index) => ({ label: step, detail: index === 0 ? template.suggestedCommand : "Pending user action.", status: index === 0 ? "complete" : "pending" })), [{ label: "Run analysis", command: "Run analysis" }, { label: "Run simulation", command: "Run simulation" }, { label: "Generate report", command: "Generate report" }, { label: "Validate workflow", command: "Validate workflow" }], "local", "completed", mechanism)]);
    appendActivity(setActivityLog, "mission", `${template.shortTitle} template loaded.`, "success");
    window.localStorage.setItem(firstRunStorageKey, "true"); setGuideOpen(false);
  }, [selectedMechanism]);
  ...
  const runAgentCommand = useCallback(async (command: string, modelProvider: V2ProviderId = activeProvider) => {
    ...
    if (normalizedCommand.includes("open parameter")) setActiveArtifact("parameters");
    if (normalizedCommand.includes("open canvas")) setActiveArtifact("canvas");
    if (normalizedCommand.includes("open validation")) { setActive("Validation"); setActiveArtifact("validation"); }
    if (intent === "report") { setActive("Reports"); setActiveArtifact("report"); }
    if (normalizedCommand.includes("supervisor") || normalizedCommand.includes("academic report") || normalizedCommand.includes("project report") || normalizedCommand.includes("copy report markdown")) { setActive("Reports"); setActiveArtifact("report"); appendActivity(setActivityLog, "report", "Supervisor Submission Mode opened for academic report preparation.", "success"); }
    ...
    setMessages((thread) => [...thread, buildAgentMessage(buildNonSolverReply(intent, missing, solverResult, command), intent, preSteps, defaultActions)]);
...
function buildNonSolverReply(intent: V2AgentIntent, missing: string[], solverResult: Record<string, unknown> | null, command = "") { if (intent === "missing_parameters") return missing.length ? `Missing or invalid parameters: ${missing.join(", ")}.` : "No required numeric parameters appear to be missing."; if (intent === "report") return /supervisor|academic|project|copy report markdown/i.test(command) ? "Opening Supervisor Submission Mode. The academic report preview will use deterministic solver evidence where available and mark missing evidence clearly." : solverResult ? "Opening the report builder with the available deterministic result context." : "Opening Reports. Run an analysis first for report-ready numerical content."; if (intent === "synthesize") return solverResult ? "Open Results and use synthesis recommendations to suggest iteration direction from solver output." : "Run analysis or simulation first; synthesis recommendations need deterministic result context."; if (intent === "validate") return "Opening validation context. This documents solver boundaries and source-of-truth rules."; return "I can run analysis, run a sweep simulation, generate a report, recommend improvement direction, or list missing parameters."; }
