...
import type { V2AgentMessage, V2ArtifactKind, V2MechanismState } from "@/components/v2/types";
...
export function V2ArtifactDetail({ kind, state, messages, onCommand }: { kind: V2ArtifactKind; state: V2MechanismState; messages: V2AgentMessage[]; onCommand: (command: string) => void }) {
...
export function V2ReportArtifactDetail({ state, messages }: { state: V2MechanismState; messages: V2AgentMessage[] }) { return <DetailShell eyebrow="Report draft" title="Report builder and export"><p className="mb-4 rounded-2xl border border-v2-border bg-[#0d0c0b] p-3 text-sm text-v2-muted">Report can be generated from current mission evidence.</p><SectionList items={["Current mission evidence", "Deterministic results", "Sweep data", "Synthesis recommendation", "Report preview", "Export actions"]} /><div className="mt-4 rounded-2xl border border-v2-border bg-[#0d0c0b] p-3"><ReportPreviewPanel mechanismType={state.selectedMechanism} inputParameters={state.inputParameters} solverResult={state.solverResult} sweepResult={state.sweepResultForReport} agentWorkflow={{ messages }} synthesisRecommendations={state.latestSynthesisRecommendations as Record<string, unknown> | null} /></div></DetailShell>; }
