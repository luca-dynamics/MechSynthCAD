...
export function V2ArtifactDrawer({ open, title, subtitle, onClose, children }: DrawerProps) {
  if (!open) return null;
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/70 backdrop-blur-sm" role="dialog" aria-modal="true">
    <button aria-label="Close artifact drawer" className="absolute inset-0 cursor-default" onClick={onClose} />
    <aside className="v2-scrollbar relative h-full w-full max-w-5xl overflow-y-auto border-l border-v2-border bg-[#090807] shadow-2xl">
      <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-v2-border bg-[#090807]/95 px-5 py-4 backdrop-blur">
        <div><p className="text-[11px] font-bold uppercase tracking-[0.28em] text-amber-500">V2 mission artifact</p><h2 className="mt-1 text-xl font-semibold text-v2-text">{title}</h2>{subtitle ? <p className="mt-1 text-sm text-v2-muted">{subtitle}</p> : null}</div>
        <button onClick={onClose} className="rounded-xl border border-v2-border bg-[#151311] px-3 py-2 text-xs font-semibold text-v2-muted hover:text-amber-500">Close</button>
      </div>
...
export function V2ArtifactDetail({ kind, state, messages, onCommand }: { kind: ArtifactKind; state: V2MechanismState; messages: V2AgentMessage[]; onCommand: (command: string) => void }) {
  ...
  if (kind === "report") return <V2ReportArtifactDetail state={state} messages={messages} />;
  if (kind === "validation") return <V2ValidationArtifactDetail />;
  return <V2SynthesisArtifactDetail state={state} onCommand={onCommand} />;
}

export function V2CompactInspector({ state, onOpenArtifact, onCommand }: { state: V2MechanismState; onOpenArtifact: (kind: ArtifactKind) => void; onCommand: (command: string) => void }) {
  ...
  return <div className="space-y-2 text-xs">
    ...
    <div className="grid grid-cols-2 gap-2 pt-2">
      <Action onClick={() => onOpenArtifact("parameters")}>Edit parameters</Action><Action onClick={() => onCommand("Run analysis")}>Run analysis</Action><Action onClick={() => onCommand("Run simulation")}>Run simulation</Action><Action onClick={() => onOpenArtifact("result")}>Open artifacts</Action>
    </div>
  </div>;
}
...
function Blocked({ text, action, onAction }: { text: string; action: string; onAction: () => void }) { return <div className="rounded-[1.35rem] border border-dashed border-v2-border bg-[#11100f] p-6 text-v2-muted"><p>{text}</p><Action onClick={onAction}>{action}</Action></div>; }
