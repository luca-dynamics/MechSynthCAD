...
function hasCurrentResult(state: V2MechanismState) { return state.selectedMechanism === "four_bar" ? Boolean(state.displayResult || state.result) : Boolean(state.displaySliderCrankResult || state.sliderCrankResult); }
function hasCurrentSweep(state: V2MechanismState) { return state.selectedMechanism === "four_bar" ? Boolean(state.sweepResult) : Boolean(state.sliderCrankSweepResult); }
...
export function V2CompactInspector({ state, onOpenArtifact, onCommand }: { state: V2MechanismState; onOpenArtifact: (kind: V2ArtifactKind) => void; onCommand: (command: string) => void }) {
  ...
  const hasSweep = hasCurrentSweep(state);
  const hasResult = hasCurrentResult(state);
  return <div className="space-y-2 text-xs">
    ...
    <Row k="Result" v={hasResult ? "Available" : "Not generated"} />
    <Row k="Simulation" v={hasSweep ? "Samples available" : "Not run"} />
    <div className="grid grid-cols-2 gap-2 pt-2">
      <Action onClick={() => onOpenArtifact("parameters")}>Edit parameters</Action><Action onClick={() => onCommand("Run analysis")}>Run analysis</Action><Action onClick={() => onCommand("Run simulation")}>Run simulation</Action><Action onClick={() => hasResult ? onOpenArtifact("result") : onCommand("Run analysis")}>{hasResult ? "Open result" : "Run analysis first"}</Action><Action onClick={() => hasSweep ? onOpenArtifact("simulation") : onCommand("Run simulation")}>{hasSweep ? "Open simulation" : "Run simulation first"}</Action><Action onClick={() => onOpenArtifact("report")}>Open report</Action>
    </div>
  </div>;
}
...
export function V2ResultArtifactDetail({ state, onCommand }: { state: V2MechanismState; onCommand: (command: string) => void }) {
  ...
  if (!result) return <Blocked text="Run deterministic analysis to generate numerical evidence." action="Run analysis" secondaryAction="Open parameters" onAction={() => onCommand("Run analysis")} onSecondaryAction={() => onCommand("Open parameter intake")} />;
...
function Action({ children, onClick }: { children: React.ReactNode; onClick: () => void }) { return <button onClick={onClick} className="min-h-10 rounded-xl border border-v2-border bg-[#191715] px-3 py-2 text-xs font-semibold text-v2-muted hover:border-amber-600/40 hover:text-amber-500 focus-visible:outline focus-visible:outline-2 focus-visible:outline-amber-500/60">{children}</button>; }
...
function Blocked({ text, action, onAction, secondaryAction, onSecondaryAction }: { text: string; action: string; onAction: () => void; secondaryAction?: string; onSecondaryAction?: () => void }) { return <div className="rounded-[1.35rem] border border-dashed border-v2-border bg-[#11100f] p-6 text-v2-muted"><p className="text-sm leading-6">{text}</p><div className="mt-4 flex flex-wrap gap-2"><Action onClick={onAction}>{action}</Action>{secondaryAction && onSecondaryAction ? <Action onClick={onSecondaryAction}>{secondaryAction}</Action> : null}</div></div>; }
