...
export function V2SupervisorSubmissionPanel({ state, messages, providers = [], onCommand }: { state: V2MechanismState; messages: V2AgentMessage[]; providers?: V2ProviderStatus[]; onCommand: (command: string) => void }) {
  ...
  useEffect(() => { const raw = window.localStorage.getItem(storageKey); if (raw) setMetadata({ ...defaultAcademicReportMetadata(), ...JSON.parse(raw) }); }, []);
  useEffect(() => { window.localStorage.setItem(storageKey, JSON.stringify(metadata)); }, [metadata]);
  const report = useMemo(() => buildAcademicReport({ state, messages, metadata, providers }), [state, messages, metadata, providers]);
  async function copyMarkdown() { await navigator.clipboard.writeText(report.markdown); setCopyStatus("Markdown copied for supervisor report."); window.setTimeout(() => setCopyStatus(null), 2200); }
  ...
  return <section className="space-y-4">
    ...
    <details className="rounded-2xl border border-v2-border bg-[#0d0c0b] p-4"><summary className="cursor-pointer text-sm font-semibold text-v2-muted">Markdown report source</summary><pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap text-xs text-stone-400">{report.markdown}</pre></details>
  </section>;
}
